...
<div id="wrapper">
	...
	<div id="upper-right">
		...
		<?php // Code for getting the RSS-news-feed
			$rss = new DOMDocument();
			$rss->load('http://feeds.reuters.com/reuters/entertainment'); // Specify the address to the feed
			$feed = array();
	...
	<div id="bottom">
		...
		<?php // Depending on the hour of the day a different message is displayed.
			$now = date('H');
				...
				else if (($now >= 12) and ($now < 14)) echo 'Lunch time';
				else if (($now >= 14) and ($now < 17)) echo 'Home time';
				else if (($now >= 17) and ($now < 20)) echo 'Dinner time';
				else if (($now >= 20) and ($now < 22)) echo 'Have a good night ';
				else if (($now >= 22) and ($now < 23)) echo 'Sleep well';
				else if (($now >= 00) and ($now < 06)) echo 'Its late you should be asleep';
			?>
		</h3>
	</div>
</div>
